...
makeMBOInfillCritUACB = function(cb.lambda = NULL, 
                                 cb.rho = NULL,
                                 cb.alpha = NULL,
                                 base_kernel= NULL, 
                                 imprecision= NULL, 
  ...
  makeMBOInfillCrit(
    fun = function(points, models, control, par.set, designs, iter, progress, attributes = FALSE) {
      ...
      # actual AF
      res = maximize.mult * p$response - cb.lambda * p$se - cb.rho * model_imprecision + cb.alpha * noise_proxy
    ...
    name = "Uncertainty Aware Confidence bound",
    id = "uacb",
    components = c("se", "mean", "lambda", "rho", "alpha", "imprecision", "noise"),
    params = list(cb.lambda = cb.lambda, cb.rho = cb.rho, imprecision = imprecision,
                  cb.alpha = cb.alpha, noise_proxy_fun = noise_proxy_fun),
